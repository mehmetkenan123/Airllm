#!/usr/bin/env python3
# -*- coding: utf-8 -*-
""" AirLLM Studio v1.3.0 - LM Studio Style Modern Single-File UI Solution: Backend + AI Engine + Frontend """
...
def generate_response_stream(prompt, max_tokens=512, temperature=0.7):
    if not AIRLLM_AVAILABLE or model is None:
        # Simulation Mode
        response_text = f"Responding with AirLLM Studio v1.3. Model: {current_model_name or 'Ready'}.\n\nUser: {prompt}\n\nThis is a simulation response. This message appears when the actual model is not loaded. For real usage, run 'pip install airllm torch transformers'."
...
<div class="sidebar">
    <div class="sidebar-header">
        <span>AirLLM Studio+</span>
        <button class="new-chat-btn" onclick="startNewChat()">New Chat</button>
    </div>
    <ul class="history-list" id="historyList">
        <li class="history-item">Default Conversation</li>
    </ul>
</div>
...
<div class="main-content">
    ...
    <div class="chat-container" id="chatContainer">
        <div class="message ai-message">
            ...
            <div class="message-content">Hello! Welcome to AirLLM Studio v1.3.</div>
        </div>
    </div>
...
<div class="settings-panel">
    ...
    <div class="setting-group">
        <label class="setting-label">GPU Layers</label>
        <input type="number" id="gpuLayers" class="setting-input" value="50">
    </div>
    <div class="setting-group">
        <label class="setting-label">Temperature: <span id="tempVal">0.7</span></label>
        <input type="range" id="temperature" class="range-slider" min="0" max="2" step="0.1" value="0.7" oninput="updateVal('tempVal', this.value)">
    </div>
    <div class="settings-title" style="margin-top:30px;">System</div>
    <div class="setting-group" style="font-size:13px;">
        <p>RAM: <span id="ramUsage">-%</span> | CPU: <span id="cpuUsage">-%</span></p>
    </div>
</div>
...
<script>
    ...
    async function fetchModels() {
        try {
            ...
            if (data.models && data.models.length > 0) {
                ...
                select.innerHTML = '<option value="sim">Simulation Mode</option>';
            }
    ...
    function appendMessage(role, text) {
        ...
        div.innerHTML = '<div class="avatar ' + role + '">' + (role === 'user' ? 'You' : 'AI') + '</div><div class="message-content">' + text + '</div>';
    ...
    async function sendMessage() {
        ...
        } catch (error) { aiDiv.innerText = "Error: " + error.message; }
        finally { isGenerating = false; sendBtn.disabled = false; statusIndicator.innerText = "● Ready"; userInput.focus(); }
    }

    function startNewChat() { chatContainer.innerHTML = ''; appendMessage('ai', 'New chat started.'); }
    function changeModel() { statusIndicator.innerText = "● Loading..."; setTimeout(() => { statusIndicator.innerText = "● Ready"; }, 1000); }
